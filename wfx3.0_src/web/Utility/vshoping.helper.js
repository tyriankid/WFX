...
function submmitorder() {
    ...
    $.ajax({
        ...
        type: 'post', dataType: 'json', timeout: 10000,
        data: {
            action: "Submmitorder", shippingType: $("#selectShippingType").val(), paymentType: $("#selectPaymentType").val(), couponCode: $("#selectCoupon").val(), redpagerid: $("#selectRedPager").val(), shippingId: $('#selectShipTo').val(),
            productSku: getParam("productSku"), buyAmount: getParam("buyAmount"), from: getParam("from"), shiptoDate: $("#selectShipToDate").val(), groupbuyId: $('#groupbuyHiddenBox').val(), remark: $('#remark').val()
        },
        success: function (resultData) {
            if (resultData.Status == "OK") {
