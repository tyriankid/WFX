...
namespace Hidistro.UI.Web.API
{
    public class VshopProcess : System.Web.IHttpHandler, System.Web.SessionState.IRequiresSessionState
    {
        ...
        private void ProcessSubmmitorder(System.Web.HttpContext context)
        {
            ...
            decimal pcThirdPayCash = context.Request["thirdPayCash"].ToDecimal();//pc点餐第三方实付金额
            decimal pcThirdPayDiscount = context.Request["thirdPayDiscount"].ToDecimal();//pc点餐第三方优惠金额
            ...
            else
            {
                ...
                else if (result == 99)//addby hj 20151227如果等于99,则是门店配送功能(子门店配送,将modename换成门店名)
                {
            ...
            for (int i = 0; i < type.Rows.Count; i++)
            {
                ...
                if (subTotal != new decimal(0))
                {
                    ...
                    if (allFull.Rows.Count > 0)//如果存在活动时进入
                    {
                        ...
                        //pc端点餐判断
                        ManagerInfo currentManager = ManagerHelper.GetCurrentManager();
                        int currentOrderStoreId = 0;//当前订单的门店id
                        int currentActivityStoreId = 0;//当前活动的门店id
                        int modenameStoreId = ManagerHelper.getClientUserIdByModeName(order.ModeName);
                        if (currentManager != null && currentManager.ClientUserId > 0) //如果是子门店在pc端点餐,活动id(storeid)匹配当前管理员的clientuserid
                        {
                            currentOrderStoreId = currentManager.ClientUserId;
                        }
                        //微信端点餐判断
                        else if ( modenameStoreId>0)//如果order的modename为子门店的门店名,那么活动id(storeid)需要匹配当前门店的userid
                        {
                            currentOrderStoreId = modenameStoreId;
                        }
                        ...
                        while (true)
                        {
                            ...
                            else if (subTotal >= decimal.Parse(allFull.Rows[allFull.Rows.Count - 1]["MeetMoney"].ToString()))
                            {
                                ...
                                currentActivityStoreId = allFull.Rows[allFull.Rows.Count - 1]["storeid"].ToInt();
                            ...
                            else if (subTotal <= decimal.Parse(allFull.Rows[0]["MeetMoney"].ToString()))
                            {
                                ...
                                currentActivityStoreId = allFull.Rows[0]["storeid"].ToInt();
                            ...
                            else
                            {
                                if (subTotal >= decimal.Parse(allFull.Rows[num4]["MeetMoney"].ToString()))
                                {
                                    ...
                                    currentActivityStoreId = allFull.Rows[num4]["storeid"].ToInt();
