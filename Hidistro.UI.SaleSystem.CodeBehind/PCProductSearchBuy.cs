namespace Hidistro.UI.SaleSystem.CodeBehind
{
    ...
    public class PCProductSearchBuy : VshopTemplatedWebControl
    {
        ...
        private System.Web.UI.HtmlControls.HtmlInputHidden defaultShippingtype;
        private System.Web.UI.HtmlControls.HtmlInputHidden txtStoreId;
        private System.Web.UI.HtmlControls.HtmlInputHidden txtThirdWayDiscountInfo;
        ...
        private void rptProducts_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if ((e.Item.ItemType == ListItemType.Item) || (e.Item.ItemType == ListItemType.AlternatingItem))
            {
                Literal literal = (Literal)e.Item.Controls[0].FindControl("litpromotion");//用于展示满减信息的literal
                string str = "";
                ...
                if (!string.IsNullOrEmpty(str))
                {
                    ...
                    {
                        literal.Text = "<span class=\"sale-favourable\"><i>满" + decimal.Parse(defaultView[0]["MeetMoney"].ToString()).ToString("0") + "</i><i role='discountText'>减" + decimal.Parse(defaultView[0]["ReductionMoney"].ToString()).ToString("0") + "</i></span>";
                    }
                    else
                    {
