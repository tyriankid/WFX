...
$(function () {
    ...
    //    for (var i = 0; i < currentCate.top.length; i++) {
    //        if (document.body.scrollTop == currentCate.top[i]) {
    //            $("#categoryList li").removeClass("activeLi");
    //            $("[categoryid=" + currentCate.ary[i] + "]").addClass("activeLi");
    //            $(".categoryList").scrollTop($("#categoryList li").height() * i);
    //        }
    //    }
    //})
...
function init() {
    ...
    localStorage.clear();
    localStorage.setItem("proLa", "proLa");
}
    ...
    $.ajax({
        ...
        success: function (e) {
            if (e.state === 0) {
                ...
                $("#goodsList>ul>li").eq(currentCate.index).find("ul").append(productsLi);
                setTimeout(function () {
                    currentCate.flag = true;
                },1000)
            }
            else if(e.state === 1){
                //没有商品
        ...
        success: function (e) {
            if (e.state === 0) {
                ...
                for (var i = 0; i < e.data.length; i++) {
                    ...
                    currentCate.ary.push(e.data[i].CategoryId);
                ...
                $("#goodsList>ul").append(categoryBoxLi);
            }
            else {
                alert("error");
            }
...
function getProductsList(target, categoryId, name) {
    currentCate.index = $(target).index();
    var ul = $("#goodsList>ul>li").eq(currentCate.index).find("ul");
    $(target).addClass("activeLi").siblings().removeClass("activeLi");
    var scrH = 0;
    for (var i = 0; i < currentCate.index; i++) {
        scrH += $("#goodsList>ul>li").eq(i).height();
    }
...
window.onscroll = function () {
    if (currentCate.ary.length == currentCate.index + 1) return false;
    if (document.body.scrollTop >= document.body.clientHeight - window.innerHeight) {
        if (currentCate.flag) {
            getProducts(currentCate.ary[currentCate.index + 1], 'PRO');
            $("#categoryList li").removeClass("activeLi");
            $("[categoryid=" + currentCate.ary[currentCate.index + 1] + "]").addClass("activeLi");
            currentCate.index += 1;
            $(".categoryList").scrollTop($("#categoryList li").height() * currentCate.index);
            currentCate.top.push(document.body.scrollTop);
        }
    }
}
