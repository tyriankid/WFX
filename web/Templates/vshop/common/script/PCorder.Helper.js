...
$(function () {
    ...
    $("body").on("click", "#btnSubmmit", function () {
        ...
        //将优惠方式放到展示框内
        var strdiscount = "";
        strdiscount = strdiscount + "<div class='discountType'  >第二杯半价：<i class='discountC'>-￥" + halfbuyTotal + "</i></div>";
        strdiscount = strdiscount + "<div class='discountType'  >买一送一：<i class='discountC'>-￥" + givebuyTotal + "</i></div>";
        strdiscount = strdiscount + "<div class='discountType'  >直接优惠：<i class='discountC'>-￥" + directCutdown + "</i></div>";
        strdiscount = strdiscount + "<div class='discountType'  >满减优惠：<i class='discountC' role='discountTotal'>-￥ 计算中..</i></div>";
        $.ajax({
            url: "/API/ActivityHandler.ashx",
            type: 'post', dataType: 'json', timeout: 10000,
            data: { action: "getDiscountTotal", storeid: $("#PCProductSearchBuy_txtStoreId").val() },
            success: function (resultData) {
                if (resultData.success == true) {
                    discountTotal = resultData.discount;
                }
                getTotalPrice();
            }
        });
...
var halfbuyTotal = 0;//第二杯半价
var givebuyTotal = 0;//买一送一优惠
var directCutdown = 0;//直接优惠
var discountTotal = 0;//满减优惠
//遍历orderList的所有元素,获取商品价格并计算放入总价栏
function getTotalPrice() {
    var tPrice = 0.00;
    halfbuyTotal = 0; givebuyTotal = 0; directCutdown = 0;
    $("#orderList ul li").each(function () {
        var giveNumber = $(this).find("#giveNum").attr("value");//买一送一数量
        var halfNumber = $(this).find("#halfNum").attr("value");//第二杯半价数量
        var number = $(this).find("input[type=tel]").attr("value");//商品数量
        var singlePrice = $(this).find("b").html(); //商品单价
        tPrice = getSum(tPrice, getX(singlePrice, number, giveNumber, halfNumber));
    });
    //计算优惠券优惠
    ...
    if (parseFloat($("#txtDiscount").val()) > 0) {
        ...
        directCutdown = $("#txtDiscount").val();
    }
    //计算满减优惠
    if (discountTotal >= 0) {
        tPrice = getMin(tPrice, discountTotal);
        $("[role='discountTotal']").html("-￥" + discountTotal);
        var tt = parseFloat($("#comid").val().replace("元", "")) - parseFloat(discountTotal);
        $("#comid").val(tt + "元");
    }
...
function thirdPaySubmmitOrder() {
    if (parseFloat($("#thirdPayCash").val()) == 0 || parseFloat($("#thirdPayDiscount").val()) == 0 || !$("[chk='thirdWay']:checked").val()) {
        alert("请将第三方支付信息填写完整!");
        return false;
    }
    ThirdPayMode = "on";
...
function submmitorder() {
    ...
    var ShippingType = $('#PCProductSearchBuy_defaultShippingtype').val();//4;//自提
    var ShipTo = 1972;//深圳
...
function showMicropay() {
    alert("请在弹出的扫码支付窗口内完成扫码支付!");
    MicroPayMode = "on";
    getTotalPrice();
    submmitorder();
}
...
$("#thirdPayDiscount,#thirdPayCash")
 .keyboard({
     ...
     accepted: function (event, keyboard, el) {
         var payTotalPrice =  parseFloat($("#payYS").html().replace("￥", ""));//parseFloat(document.getElementById("priceTotal").innerHTML.replace('￥', '')); //获取总价(应收价格)
         var payDiscount = parseFloat($("#thirdPayDiscount").val());//获取优惠金额
         var payCash = parseFloat($("#thirdPayCash").val());//获取实收金额

         $("#thirdPayDiscount").val(getMin(payTotalPrice,payCash));
         $("#thirdPayCash").val(getMin(payTotalPrice, payDiscount));
...
$('#Sshi').keyboard({
     ...
     accepted: function (event, keyboard, el) {
         ...
         */
         ...
         var payValue = document.getElementById("Sshi").value;
         var payBack = payValue - xianJ1;
         document.getElementById("Sling").innerHTML = payBack.toFixed(1) + "元";
     },
     canceled: function (event, keyboard, el) {//取消显示回调函数
...
var ThirdPayMode = "off";//第三方支付订单写入功能默认为关闭
