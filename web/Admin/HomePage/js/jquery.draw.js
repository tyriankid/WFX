...
function integrate(e, j) {
	...
	$("#addAttr"+j+" input").click(function(){
	    if ($(this).attr("name") == "way" + j + "") {
	        var cls = $("#module" + j + " ul").attr("class");
			switch($(this).attr("value")){
				case "two" :
				    $("#module" + j + " ul").attr("class", cls.replace(/\d/g,'2'));
					break;
				case "three" :
				    $("#module" + j + " ul").attr("class", cls.replace(/\d/g, '3'));
					break;
				case "four" :
				    $("#module" + j + " ul").attr("class", cls.replace(/\d/g, '4'));
					break;
			}
		}
	    ...
	    if ($(this).attr("name") == "newLine" + j + "") {
	        var num = /\d/g.exec($("#module" + j + " ul").attr("class"));
	        if ($(this).attr("value") == "true") {
	            $("#module" + j + " ul").removeClass("newLine" + num);
	        } else {
	            $("#module" + j + " ul").addClass("newLine" + num);
	        }
	    }
...
function dhAttr() {
	var divhtml = "<div id='addAttr"+i+"' class='addAttr' i='"+i+"'>"+
					  "<table class='nature' cellpadding='0' cellspacing='0' border='0'>"+
                          ...
                          "<tr><td align='right'>是否换行：</td><td><input type='radio' name='newLine" + i + "' value='true' checked='checked' />是</td><td colspan='2'><input type='radio' name='newLine" + i + "' value='false' />否(当前无法显示滑动效果，真机上可滑动)</td></tr>" +
...
function addDH(e) {
	...
	var num = /\d/g.exec($("#module" + m + " ul").attr("class"));
	if ($("#addAttr" + m + " input[name='newLine" + m + "']:checked").attr("value") == "true") {
	    $("#module" + m + " ul").removeClass("newLine" + num);
	} else {
	    $("#module" + m + " ul").addClass("newLine" + num);
	}
