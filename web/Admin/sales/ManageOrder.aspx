                    ...
                    <tr class="td_bg">
                        ...
                        <td class="td_txt_cenetr">
                           &nbsp;<span class="Name" style="text-indent:0px;">
                               ...
                               <asp:Label runat="server" ID="lbDeliveryOrder"  CssClass="submit_faihuo" Visible="false">
                                   <a onclick= 'return SetDeliveryUser($(this))'  orderid='<%#Eval("OrderId") %>' runat="server" id="setDeliveryUser">ss</a>
                               </asp:Label>
...
<%--     <!-- 打印机控件引用-->
    ...
    <script type="text/javascript">       
        ...
        //分配配送员
        function SetDeliveryUser(e) {
            if (e.html() == "配送") {
                DialogFrame("product/SendDeliveryUser.aspx?orderid=" + e.attr("orderid"), "配送员分配", null, null);
            }
            else if (e.html() == "送达") {
                $.ajax({
                    type: "POST",   //访问WebService使用Post方式请求
                    contentType: "application/json", //WebService 会返回Json类型
                    url: "ManageOrder.aspx/ArrivedOrder", //调用WebService的地址和方法名称组合 ---- WsURL/方法名
                    data: "{OrderId:'" + e.attr("orderid") + "'}",//这里是要传递的参数，格式为 data: "{paraName:paraValue}",下面将会看到      
                    dataType: 'json',
                    success: function (result) { //回调函数，result，返回值
                        if (result.d == "true") {
                            alert("操作成功！该订单现已送达！");
