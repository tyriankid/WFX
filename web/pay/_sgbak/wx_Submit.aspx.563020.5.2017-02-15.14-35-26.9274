    ...
    function jsApiCall()
    {
        ...
        <%=pay_json%>,//josn串
                    function (res)
                    {
                        ...
                        var isProTaste = localStorage.getItem("proLa");
                        ...
                        if(res.err_msg == "get_brand_wcpay_request:ok" ) {
                            if(!isProTaste){
                                ...
                                alert("订单支付成功!点击确认进入我的订单中心");
                                location.href = "/vshop/MemberCenterPro.aspx?status=3&orderid=<%=orderId%>&uid=<%=uid%>";
                            }
                        }
                        else
                        {
                            if(!isProTaste){
                                ...
                                alert("支付取消或者失败");
                                location.href = "/vshop/MemberCenterPro.aspx?status=1";
                            }
