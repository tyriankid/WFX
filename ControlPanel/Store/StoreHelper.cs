...
using System.Xml;
    ...
    public static class StoreHelper
    {
        ...
        public static DataTable GetDeliveryMemberList(string where="")
        {
            return new StoreDao().GetDeliveryMemberList(where);
        }
        ...
        public static DeliveryOrderInfo GetDeliveryOrderInfoByOrderId(string orderid)
        {
            return new StoreDao().GetDeliveryOrderInfoByOrderId(orderid);
        }

        public static bool SetDeliveryUser(DeliveryOrderInfo info)
        {
            if(new StoreDao().SetDeliveryUser(info) && new StoreDao().UpdateDeliveryUserState(info.DeliveryUserId,1)) //配送中
            {
                return true;
            }
            else{
        ...
        public static string getOrderDeliveryState(string orderid)
        {
            return new StoreDao().getOrderDeliveryState(orderid);
        }

        public static bool arrivedDeliveryOrder(DeliveryOrderInfo info)
        {
            if (new StoreDao().arrivedDeliveryOrder(info) && new StoreDao().UpdateDeliveryUserState(info.DeliveryUserId, 0))//回归为闲置状态
            {
                return true;
            }
            else
