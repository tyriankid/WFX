namespace Hidistro.SqlDal.Store
{
    ...
    public class MessageDao
    {
        ...
        //微信端点餐通过当前modename获取门店id
        public int getClientUserIdByModeName(string modename)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("select ClientUserId from aspnet_Managers am right join aspnet_distributors ad on ad.userid=am.clientuserid where  ad.storename = '" + modename + "'");
            object returnStr = this.database.ExecuteScalar(sqlStringCommand);
            return returnStr.ToInt();
        }
