...
using System.Text.RegularExpressions;
    ...
    public class ProductBrowseDao
    {
        ...
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("select ReductionMoney,ActivitiesId,ActivitiesName,MeetMoney,ActivitiesType,ActivitiesDescription,storeid from Hishop_Activities where datediff(dd,GETDATE(),StartTime)<=0 and datediff(dd,GETDATE(),EndTIme)>=0 and (ActivitiesType=0 or ActivitiesType=" + ActivitiesType + ")  order by MeetMoney asc");
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
            return this.database.ExecuteDataSet(sqlStringCommand).Tables[0];
        }
        ...
        public DataTable GetProductsRange(MemberInfo member, int? topicId, int BrandId, int? categoryId, int distributorId, string keyWord, int pageNumber, int maxNum, out int toal, string sort, bool isAsc = false, ProductInfo.ProductRanage productRanage = ProductInfo.ProductRanage.NormalSelect, string swr = "", int rangeType = 0,int storeid=0)
        {
            int discount = 100;
            StringBuilder builder = new StringBuilder();
            builder.AppendFormat("ProductId,CategoryId,SkuId,Stock,ProductName,ProductCode,MainCategoryPath,ShowSaleCounts AS SaleCounts,ShortDescription,BrandId,", maxNum);
