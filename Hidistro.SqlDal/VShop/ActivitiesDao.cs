namespace Hidistro.SqlDal.VShop
{
    ...
    public class ActivitiesDao
    {
        ...
        public int AddActivities(ActivitiesInfo activity)
        {
            ...
            builder.Append("INSERT INTO Hishop_Activities(").Append("ActivitiesName,ActivitiesType,MeetMoney,ReductionMoney,StartTime,EndTIme,ActivitiesDescription,Type,StoreId)").Append(" VALUES (").Append("@ActivitiesName,@ActivitiesType,@MeetMoney,@ReductionMoney,@StartTime,@EndTime,@ActivitiesDescription,@Type,@StoreId)");
            ...
            this.database.AddInParameter(sqlStringCommand, "StoreId", DbType.Int32, activity.StoreId);
        ...
        public DbQueryResult GetActivitiesList(ActivitiesQuery query)
        {
            ...
            return DataHelper.PagingByRownumber(query.PageIndex, query.PageSize, query.SortBy, query.SortOrder, query.IsCount, "Hishop_Activities ", "ActivitiesId", (builder.Length > 0) ? builder.ToString() : null, "*, (SELECT Name FROM Hishop_Categories WHERE CategoryId = Hishop_Activities.ActivitiesType) AS CategoriesName,(select storename from aspnet_distributors where userid = Hishop_Activities.storeid) as StoreName");
        }
        ...
        {
            StringBuilder builder = new StringBuilder();
            builder.Append("UPDATE Hishop_Activities SET ").Append("StoreId=@StoreId,").Append("ActivitiesName=@ActivitiesName,").Append("ActivitiesType=@ActivitiesType,").Append("MeetMoney=@MeetMoney,").Append("ReductionMoney=@ReductionMoney,").Append("StartTime=@StartTime,").Append("EndTIme=@EndTIme,").Append("ActivitiesDescription=@ActivitiesDescription").Append(" WHERE ActivitiesId=@ActivitiesId");
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand(builder.ToString());
            this.database.AddInParameter(sqlStringCommand, "ActivitiesName", DbType.String, activity.ActivitiesName);
            this.database.AddInParameter(sqlStringCommand, "ActivitiesType", DbType.Int32, activity.ActivitiesType);
            this.database.AddInParameter(sqlStringCommand, "MeetMoney", DbType.Decimal, activity.MeetMoney);
