namespace Hidistro.SqlDal.Members
{
    ...
    public class MemberGradeDao
    {
        ...
        public MemberGradeInfo GetMemberNextGrade(int userId)
        {
            DbCommand sqlStringCommand = this.database.GetSqlStringCommand("select top 1 * from aspnet_MemberGrades where points > (SELECT mg.Points FROM aspnet_MemberGrades MG left join aspnet_Members M on M.gradeid=MG.gradeid WHERE M.userid  = @UserId) order by Points");
